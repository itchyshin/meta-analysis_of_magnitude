########################################################################
##  lnM  – simulation, summary & graphics (delta vs SAFE with BCa)
##  24 independent + 12 paired parameter sets (36 total)
##  ASCII-only code, works on Linux / macOS / Windows
##  last tested: 25-Jun-2025  (R ≥ 4.3, MASS 7.3-60, boot 1.3-28, ggplot2 3.5-0)
########################################################################

library(MASS)       # mvrnorm()
library(parallel)   # mclapply / parLapply
library(boot)       # for BCa correction
...
lnM_delta_ind <- function(x1, x2, s1, s2, n1, n2) {
  ...
  sD2    <- s1^2/n1 + s2^2/n2
  varMSB <- h^2*(2*sD2^2 + 4*sD2*(x1-x2)^2)
  varMSW <- 2*MSW^2/(n1+n2-2)
  g1 <- 0.5/Δ
  g2 <- -0.5*MSB/(Δ*MSW)
  v  <- posify(g1^2*varMSB + g2^2*varMSW)
  c(pt=pt, var=v)
}
lnM_delta_dep <- function(x1, x2, s1, s2, n, rho) {
  ...
  varMSB <- (n/2)^2*(2*sD2^2/n^2 + 4*(x1-x2)^2*sD2/n)
  varMSW <- (s1^4 + s2^4 + 2*rho^2*s1^2*s2^2)/(2*(n-1))
  g1 <- 0.5/Δ
  g2 <- -0.5*MSB/(Δ*MSW)
  v  <- posify(g1^2*varMSB + g2^2*varMSW)
  c(pt=pt, var=v)
...
## =====================================================================
##  2.  SAFE bootstrap – BCa bias correction
## =====================================================================
## -- helper to extract 50th pctile for boot()
.bca_stat <- function(dat, i) {
  median(dat[i,"cloud"], na.rm=TRUE)
}
...
safe_ind <- function(x1bar,x2bar,s1,s2,n1,n2,B=1e4,chunk=5e3) {
  ...
  cloud <- cloud[1:B]
  # BCa
  df  <- data.frame(cloud=cloud)
  bb  <- boot(df, .bca_stat, R=2000)
  ci  <- boot.ci(bb, type="bca")
  pt  <- ci$bca[4]    # 50th-pctile bias-corrected
...
## …then similarly re-plot relbias and coverage as before…