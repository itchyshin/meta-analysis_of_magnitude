...
## tested: 25-Jun-2025  (R ≥ 4.3, MASS 7.3-60, ggplot2 3.5-0)
########################################################################
...
## Only the kernel name changed (lnM_core); otherwise identical logic.
...
runner <- function(i) {
  ...
  tv_s <- var(Mok["safe_pt", ])            # Monte-Carlo truth
...
n_cores_use <- max(1L, parallel::detectCores() - 92)
...
write.csv(results, "lnM_SAFE14_results.csv", row.names = FALSE)
# read.csv("lnM_SAFE14_results.csv", stringsAsFactors = FALSE)
