...
## =====================================================================
##  2.  SAFE bootstrap – **double-truncation removed**           (FIX-5)
## =====================================================================
## ---------- 2a. SAFE bootstrap – *independent* groups -----------------
                     ...
                     B = 1e4, chunk = 5e3)
{
  ...
  while (length(cloud) < B) {
    ...
    okV <- d[, 3] > 0 & d[, 4] > 0          # keep only +ve variances
    d   <- d[okV, , drop = FALSE]
    if (!nrow(d)) next                      # nothing in this chunk
    ...
    ok   <- MSB > MSW                       # **single truncation ONLY**
    kept <- kept + sum(ok)
    if (!any(ok)) next
    
    Delta <- MSB[ok] - MSW[ok]              # strictly positive
    cloud <- c(cloud,
               0.5 * (log(Delta) -
                        log(2 * n1 * n2 / (n1 + n2)) -
                        log(MSW[ok])))
...
## ---------- 2b. SAFE bootstrap – *paired* groups ----------------------
...
## ---------- 5.  simulation driver  (robust summaries) -----------------
set.seed(20250625)
...
runner <- function(i) {
  ...
  ## ---------- “true” Monte-Carlo variances (all draws) -----------------
  true_var_delta <- var(M["delta_pt", ], na.rm = TRUE)
  true_var_safe  <- var(M["safe_pt",  ], na.rm = TRUE)
  
  ## ---------- keep only successful point estimates --------------------
  ok   <- !is.na(M["delta_pt", ])
  Mok  <- M[, ok, drop = FALSE]
  
  ## ---------- OPTIONAL: drop the top 1 % extreme variance estimates ---
  ## keep_hi <- Mok["delta_var", ] < quantile(Mok["delta_var", ], 0.99)
  ## Mok     <- Mok[, keep_hi, drop = FALSE]
  
  ## ---------- ROBUST summaries of variance estimators -----------------
  mean_var_delta   <- mean(   Mok["delta_var", ])
  mean_var_safe    <- mean(   Mok["safe_var",  ])
  med_var_delta    <- median( Mok["delta_var", ])
  med_var_safe     <- median( Mok["safe_var",  ])
  ...
  data.frame(
    ...
    med_var_delta  = med_var_delta,
    med_var_safe   = med_var_safe,
    trim_var_delta = trim_var_delta,
    trim_var_safe  = trim_var_safe,
