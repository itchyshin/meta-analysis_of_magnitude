...
## internal small lnM for bootstrap
...
safe_ind <- function(x1bar, x2bar, s1, s2, n1, n2,
  ...
  # first truncation on observed Δ0
  if (MSB0 <= MSW0) {
  ...
  while (k < B) {
    ...
    # second truncation on each bootstrap Δ*
    use <- MSB > MSW
    if (!any(use)) next
    ...
    idx      <- seq_len(take_len)
    cloud[(k + 1):(k + take_len)] <- vals[idx]
    k <- k + take_len
  }
...
safe_dep <- function(x1bar, x2bar, s1, s2, n, rho,
  ...
  Sig[1,2] <- rho*s1*s2/n; Sig[2,1] <- rho*s1*s2/n
  Sig[3,3] <- 2*s1^4/(n-1); Sig[4,4] <- 2*s2^4/(n-1)
  Sig[3,4] <- 2*rho^2*s1^2*s2^2/(n-1)
  Sig[4,3] <- Sig[3,4]
