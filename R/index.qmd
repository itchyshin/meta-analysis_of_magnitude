...
We demonstrate a complete workflow for quantifying effect magnitude using $lnM$. For each dataset we compute a conventional standardized mean difference (SMD), then show how taking the absolute value produces a folded-normal quantity whose sampling distribution is not Gaussian.

We then compute $lnM$ in two ways:
...
We then fit multilevel meta-analytic models and meta-regressions and visualize results using the same plot types used in the manuscript.
...
Part II — Simulation reproduction (Figures + Tables). We load pre-computed simulation summaries and reproduce bias, variance performance, coverage, root mean square error (RMSE), and Monte Carlo standard error (MCSE) diagnostics. We also generate compact tables summarizing feasibility (fail rates), stability, and SAFE acceptance rates.
...
Almeida *et al.* asked whether experimental manipulations of nutrition produce consistently large departures from control conditions across physiological and performance traits. Because the biological interest is in effect magnitude rather than direction, this dataset provides a natural example for comparing absolute SMD-based measures with $lnM$.
...
## Folded-normal magnitude of SMD (a principled alternative to naive absolute d)
...
The Delta plug-in estimator of lnM can be undefined for some effect sizes (most commonly when the ANOVA components imply $MS_B \le MS_W$). SAFE is designed to be more robust in this boundary region by using a single-fit parametric bootstrap. The table below summarizes (i) how often each method produces a usable lnM point estimate and variance and (ii) how often SAFE “rescues” cases where the Delta plug-in fails.
...
We fit the same multilevel random-effects structure for two magnitude metrics: (i) the folded-normal magnitude of SMD and (ii) lnM estimated with SAFE. In both cases, effect sizes are nested within studies (random intercepts for *Study* and for the within-study effect-size ID).
...
This model summarizes the typical *magnitude* of effects using the folded-normal transformation of SMD. Note that this is not equivalent to taking |SMD|; it uses the implied folded-normal distribution to reduce the upward bias introduced by absolute values.
...
This model summarizes the typical *magnitude* of effects using lnM estimated via SAFE. SAFE is used here because lnM can be undefined under the Delta plug-in approach for some effect sizes, particularly near the boundary where between-group variability is not larger than within-group variability.
...
::: callout-tip
Because lnM is designed to behave more like a Gaussian response variable than absolute-value measures, it supports standard meta-regression workflows without the distributional complications induced by absolute values.
:::
::::
...
and fit models using either $1/\sqrt{n_0}$ (an SE-analogue) or $1/n_0$ (a variance-analogue). These predictors depend only on sample sizes and avoid structural dependence on the observed lnM.
...
indicating a substantially smaller average effect than conventional absolute-SMD estimates. Because the transformation is monotone, confidence intervals for $d_{\mathrm{eq}}$ can be  obtained by transforming the corresponding lnM interval endpoints.
:::
...
# 4) Standardized mean difference with unequal variances (SMDH)
...
# 5) Folded-normal magnitude of SMD
dat <- dat %>%                                   
  mutate(
    abs_d   = map2_dfr(yi_d,   vi_d,   ~ {
      out <- folded_norm(.x, .y)
...
# 7) lnM via SAFE (single-fit parametric bootstrap)
